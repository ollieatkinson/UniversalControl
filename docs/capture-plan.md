...
## Mac-To-Windows Bonjour Visibility

When native Windows Bonjour is installed, run a bidirectional Bonjour visibility
check before interpreting native-admission failures:
...
Windows should first browse:

```powershell
dns-sd -B _companion-link._tcp local
```

Then Windows should advertise a project-owned instance:
...
Windows should also resolve and connect to the exact Mac-controlled probe from a
...
This command redacts hostnames, addresses, and instance names, refuses
Apple-owned service types unless explicitly acknowledged, and sends only a small
project-owned probe payload to the Mac TCP observer.

The macOS summary is generated by
`scripts/mac/summarize-bonjour-visibility-artifact.py` and preserves only
registration status, event counts, service type, local-advertisement visibility,
expected-remote-instance yes/no, and optional TCP observer peer classes plus
length/timing/framing buckets. Do not commit raw `dns-sd` output.
