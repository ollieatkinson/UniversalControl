...
7. Capture and replay normalized events:
   ...
   `--route-config` when macOS is being tested as the input owner; it runs
   `route-events` and reports whether the captured pointer movement would
   activate the remote side and suppress local input, without native hooks or
   network. The expectation flags make the command fail when the capture does
   not cross the configured edge or forward any input. Run `--dry-run` first to
   validate JSON parsing and key/button mapping without injecting input. Only run
   full replay with a controlled foreground target. This proves the JSON
   generated by `listen-events` or `grab-events` can drive the same injector used
   by the receiver role.
