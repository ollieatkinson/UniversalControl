...
7. Capture and replay normalized events:
   ...
   `--dry-run` first to validate JSON parsing and key/button mapping without
   injecting input. Only run full replay with a controlled foreground target.
   This proves the JSON generated by `listen-events` or `grab-events` can drive
   the same injector used by the receiver role.
